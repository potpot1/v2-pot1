...
EOF
  wget -q https://${raw_proxy}/potpot1/v2-pot1/${branch}/config/03_routing.json -O /usr/local/etc/xray/03_routing.json
  wget -q https://${raw_proxy}/potpot1/v2-pot1/${branch}/config/06_outbounds.json -O /usr/local/etc/xray/06_outbounds.json
}
...
install_xray() {
  ...
  (crontab -l 2>/dev/null; echo "0 7 * * * wget -q http://localhost:8000/v2ray-rules-dat/release/geoip.dat -O /usr/local/share/xray/geoip.dat >/dev/null >/dev/null") | crontab -
  (crontab -l 2>/dev/null; echo "0 7 * * * wget -q http://localhost:8000/v2ray-rules-dat/release/geosite.dat -O /usr/local/share/xray/geosite.dat >/dev/null >/dev/null") | crontab -
...
rm_xwall() {
  if [ -f "/usr/local/bin/xray" ]; then
    wget -q https://${raw_proxy}/potpot1/v2-pot1/${branch}/tools/rm_xwall.sh -O /tmp/rm_xwall.sh && bash /tmp/rm_xwall.sh
    exit 0
  fi
}
